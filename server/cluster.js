      ...
      switch (_a.label) {
        case 0:
          return [
            ...
            inputQuery(
              'NumClusters',
              'How many clusters? Recommended Number (Max Number of CPUs): '.concat(
                numCPUs
              )
            ),
          ];
        case 1:
          ...
          return [
            ...
            inputQuery(
              'NumWorkers',
              'How many worker threads per cluster? Recommended Number: '.concat(
                maxNumberofWorkersPerCluster
              )
            ),
          ];
        case 2:
          ...
          return [
            4 /*yield*/,
            inputQuery('NumCalls', 'How many calls per thread to the server?'),
          ];
        case 3:
          numCalls = _a.sent();
          return [
...
function loadT() {
  return __awaiter(this, void 0, void 0, function () {
    ...
    return __generator(this, function (_b) {
      switch (_b.label) {
        case 0:
          if (!(cluster.isPrimary && node_worker_threads_1.isMainThread))
            return [3 /*break*/, 2];
          console.log(
        ...
        case 1:
          (_a = _b.sent()),
            (numClusters = _a.numClusters),
            (numWorkers = _a.numWorkers),
            (numCalls = _a.numCalls);
          ...
          cluster.on('online', function (worker) {
            console.log(
              'worker '.concat(worker.process.pid, ' is forked and running')
            );
          });
          ...
          cluster.on('exit', function (worker, code, signal) {
            ...
            if (exitedWorkers >= totalWorkers) {
              ...
              try {
                var dashboardHtml = (0,
                genDash_1.generateGrpcLoadTestDashboard)(cpuUsageData, eluData);
          ...
          if (node_worker_threads_1.isMainThread) {
            ...
            _loop_1 = function (i) {
              ...
              var worker = new node_worker_threads_1.Worker(__filename, {
                workerData: { numCalls: numCalls },
              });
              setInterval(function () {
                var currentMeasure = os.cpus();
                var _loop_2 = function (i_1) {
                  var idleDifference =
                    currentMeasure[i_1].times.idle -
                    lastMeasure_1[i_1].times.idle;
                  var totalDifference = Object.keys(
                    currentMeasure[i_1].times
                  ).reduce(function (total, mode) {
                    return (
                      total +
            ...
            client_1 = new grpcObj.greeterPackage.Greeter(
              '0.0.0.0:'.concat(PORT),
            ...
            runStub_1 = function () {
              client_1.sayHello(
                { name: 'Kenny' },
                { interceptors: [clientInterceptor_1.interceptor] },
                function (err, res) {
                  if (err) {
